...
void copyPlane2BufferBox(Vertex * & vertexBuffer, GLuint * & elementBuffer, unsigned int & elementStartIndex,
                      const Vertex & a, const Vertex & b, const Vertex & c, const Vertex & d);
void copyPlane2BufferTriangle(Vertex * & vertexBuffer, GLuint * & elementBuffer, unsigned int & elementStartIndex,
                      const Vertex & a, const Vertex & b, const Vertex & c);

TriangleMesh::TriangleMesh(float width, float height, float depth, QColor boxColor) {
    ...
    m_vertices.push_back(QVector3D(-0.0f*width, -0.5f*height, -0.5f*depth)); // e = 4
    m_vertices.push_back(QVector3D(-0.0f*width,  0.5f*height, -0.5f*depth)); // h = 5

    setColor(boxColor);
}

void TriangleMesh::transform(const QMatrix4x4 & transform) {
    for (QVector3D & v : m_vertices)
        v = transform*v;
}

void TriangleMesh::copy2Buffer(Vertex *& vertexBuffer, GLuint *& elementBuffer, unsigned int & elementStartIndex) const {
    ...
    copyPlane2BufferTriangle(vertexBuffer, elementBuffer, elementStartIndex,
            Vertex(m_vertices[1], cols[0]),
            Vertex(m_vertices[0], cols[0]),
            Vertex(m_vertices[4], cols[0])
        );
    ...
    copyPlane2BufferTriangle(vertexBuffer, elementBuffer, elementStartIndex,
            ...
            Vertex(m_vertices[3], cols[1]),
            Vertex(m_vertices[5], cols[1])
        );

    // back plane: g=5, e=4, h=7, g=6
    copyPlane2BufferBox(vertexBuffer, elementBuffer, elementStartIndex,
            Vertex(m_vertices[1], cols[2]),
    ...
    // left plane: 4,0,3,7
    copyPlane2BufferBox(vertexBuffer, elementBuffer, elementStartIndex,
            Vertex(m_vertices[1], cols[3]),
            Vertex(m_vertices[2], cols[3]),
